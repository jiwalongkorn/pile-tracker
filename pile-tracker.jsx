...
// ============================================================
// GRID: 25 row lines × 41 col lines — ตรงตามแปลนจริง
//
// Rows: A, A', B, B', C, C', ... L, L', M  (25 rows)
//   main rows (A-M): major gridlines
//   inter rows (A'-L'): intermediate gridlines between main rows
//
// Cols: 1, 1.1, 2, 2.1, 3, 3.1, ... 20, 20.1, 21  (41 cols)
//   main cols (1-21): major gridlines
//   inter cols (1.1-20.1): intermediate gridlines between main cols
//
// F2 = 2 piles at intersection (shown as 2 dots)
// F1 = 1 pile at intersection (shown as 1 dot)
//
// F2 positions:
//   - All main-row × main-col intersections for row A and M (edge rows)
//   - main-row × col 1, 2, 20, 21 intersections for rows B-L (middle rows)
// F1 everywhere else (that has a pile)
// Inter-row × inter-col = no pile
// ============================================================

const ROWS_META = [
  { id: "A",  type: "edge", dir: "LR" },
  { id: "A'", type: "inter", dir: "RL" },
  { id: "B",  type: "mid", dir: "LR" },
  { id: "B'", type: "inter", dir: "RL" },
  { id: "C",  type: "mid", dir: "LR" },
  { id: "C'", type: "inter", dir: "RL" },
  { id: "D",  type: "mid", dir: "LR" },
  { id: "D'", type: "inter", dir: "RL" },
  { id: "E",  type: "mid", dir: "LR" },
  { id: "E'", type: "inter", dir: "RL" },
  { id: "F",  type: "mid", dir: "LR" },
  { id: "F'", type: "inter", dir: "RL" },
  { id: "G",  type: "mid", dir: "LR" },
  { id: "G'", type: "inter", dir: "RL" },
  { id: "H",  type: "mid", dir: "LR" },
  { id: "H'", type: "inter", dir: "RL" },
  { id: "I",  type: "mid", dir: "LR" },
  { id: "I'", type: "inter", dir: "RL" },
  { id: "J",  type: "mid", dir: "LR" },
  { id: "J'", type: "inter", dir: "RL" },
  { id: "K",  type: "mid", dir: "LR" },
  { id: "K'", type: "inter", dir: "RL" },
  { id: "L",  type: "mid", dir: "LR" },
  { id: "L'", type: "inter", dir: "RL" },
  { id: "M",  type: "edge", dir: "LR" },
];

// Col labels for 41 columns
const COL_LABELS = [];
for (let c = 1; c <= 21; c++) {
...
export default function App() {
  ...
  const savePile = () => {
    if (!selPile) return;
    setPiles(prev => ({ ...prev, [selPile]: { ...prev[selPile], ...form, date: form.date || new Date().toISOString().slice(0, 10) } }));
    setSelPile(null); setForm(null);
  };

  const markCell = (ids, status) => {
    setPiles(prev => {
      const next = { ...prev };
      ids.forEach(id => { next[id] = { ...next[id], s: status, date: new Date().toISOString().slice(0, 10) }; });
      return next;
    });
  };
  ...
  // F2 orientation per ST-04 plan (C1 = แนวนอน ←→, C2 = แนวตั้ง ↑↓):
  //   แนวนอน (C1): col 1 (ci=0), col 21 (ci=40) — ทุกแถว (double vertical gridline)
  //                col 9 (ci=16), col 13 (ci=24) — เฉพาะ mid rows (B–L)
  //   แนวตั้ง (C2): col 9, 13 ใน edge rows (A, M)
  const f2IsHorizontal = (ci, rowType) => {
    if (ci === 0 || ci === 40) return true;                        // col 1, 21 → แนวนอนเสมอ
    if ((ci === 16 || ci === 24) && rowType === "mid") return true; // col 9, 13 × B–L
    return false; // col 9, 13 × A, M → แนวตั้ง
  };
  ...
  // Cell (footing) component — sits at a gridline intersection
  const Cell = ({ rowId, colIdx, pileIds, rowType }) => {
    const isF2 = pileIds.length === 2;
    const horiz = f2IsHorizontal(colIdx, rowType); // F2 layout direction per ST-04
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Mono', monospace", minHeight: "100vh", background: "#080a10", color: "#cdd1e0", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ background: "#060810", borderBottom: "1px solid #111420", padding: "10px 16px", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        ...
        <div>
          <div style={{ fontFamily: "'Sarabun',sans-serif", fontWeight: 700, fontSize: 14 }}>ติดตามการกดเสาเข็ม</div>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: "12px 14px" }}>
          <div style={{ display: "inline-block", minWidth: "max-content" }}>
            ...
            {/* ---- Build column width map ---- */}
            {(() => {
              // Pre-compute col widths: each col "slot" = cell width + gridline width
              // Main col: cell centered on gridline, with space either side
              // Inter col: smaller slot
              const dotSz = Math.round(8 * zoom);
              const gap3 = Math.round(3 * zoom);
              const pad = Math.round(3 * zoom);

              // cell width for each col index
              const cellW = (ci, rowType) => {
                const isMain = ci % 2 === 0;
                const horiz = f2IsHorizontal(ci, rowType);
                if (!isMain) return dotSz + pad * 2; // F1 always
                // F2 horizontal = 2 dots side by side
                if (horiz) return dotSz * 2 + gap3 + pad * 2;
                // F2 vertical = 1 dot wide
                return dotSz + pad * 2;
              };
              const cellH = (rowType, ci) => {
                const isMain = ci % 2 === 0;
                const horiz = f2IsHorizontal(ci, rowType);
                if (!isMain || rowType === "inter") return dotSz + pad * 2; // F1
                if (horiz) return dotSz + pad * 2; // F2 horiz = 1 dot tall
                return dotSz * 2 + gap3 + pad * 2; // F2 vert = 2 dots tall
              };

              // For header row height, use tallest cell in any main row
              const slotW = (ci) => cellW(ci);

              // Gridline thickness
              const mainGL = 1; // px for main gridline
              const interGL = 0; // inter gridlines are implied by spacing

              // Bay width (space between two main cols) = slot of inter col
              // We'll lay out: [mainGL] [main-cell-pad] [cell] [main-cell-pad] [interGL] [inter-cell-pad] [cell] [inter-cell-pad] [interGL] ...
              // Simplified: just use flexbox with explicit widths per col slot

              const LABEL_W = 30;
              const ROW_GAP = Math.round(4 * zoom);   // gap between row lines
              const INTER_ROW_GAP = Math.round(2 * zoom);

              // Column slot widths — use "edge" as reference (widest possible)
              const colSlotW = Array.from({length: 41}, (_, ci) => cellW(ci, "edge") + 4);

              // Total grid width
              const totalW = colSlotW.reduce((a, b) => a + b, 0) + LABEL_W;
              ...
              return (
                <div style={{ position: "relative" }}>
                  ...
                  {ROWS_META.map((meta, ri) => {
                    ...
                    return (
                      <div key={meta.id} style={{
                        ...
                        {/* Cells — each inside its col slot, centered */}
                  ...
                  {/* Legend */}
                  <div style={{ marginTop: 14, display: "flex", gap: 16, fontSize: 10, color: "#333c5a", alignItems: "center", flexWrap: "wrap" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <div style={{ display: "flex", flexDirection: "column", gap: 2, padding: 2, border: "1px solid #272c42", borderRadius: 2 }}>
                        <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#303654" }} />
                        <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#303654" }} />
                      </div>
                      <span>F2,C2 (แนวตั้ง — col 9, 13 แถว A, M)</span>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <div style={{ display: "flex", gap: 2, padding: 2, border: "1px solid #272c42", borderRadius: 2 }}>
                        <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#303654" }} />
                        <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#303654" }} />
                      </div>
                      <span>F2,C1 (แนวนอน — col 1, 21 ทุกแถว / col 9, 13 แถว B–L)</span>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <div style={{ padding: 2, border: "1px solid #272c42", borderRadius: 2 }}>
                        <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#303654" }} />
                      </div>
                      <span>F1 (1 ต้น)</span>
                    </div>
                    {[["#22c55e","กดแล้ว"],["#ef4444","มีปัญหา"],["#303654","ยังไม่กด"]].map(([c,l])=>(
        ...
        <div style={{ width: 255, borderLeft: "1px solid #111420", background: "#060810", display: "flex", flexDirection: "column", flexShrink: 0 }}>
          ...
          {selPile && form ? (
            <div style={{ padding: 12, borderTop: "1px solid #111420", display: "flex", gap: 7 }}>
              <button onClick={savePile} style={{ flex:1, padding:"8px", borderRadius:3, border:"1px solid #16703a", background:"#0b2117", color:"#22c55e", cursor:"pointer", fontFamily:"'Sarabun',sans-serif", fontWeight:700, fontSize:13 }}>บันทึก ✓</button>
            ...
            <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", alignItems:"center", color:"#151825", textAlign:"center", padding:20 }}>
              ...
              <div style={{ fontSize:11, fontFamily:"'Sarabun',sans-serif", lineHeight:1.9 }}>
                คลิก cell บน grid<br/>เพื่อดูหรือแก้ไขเสาเข็ม
              </div>
            </div>
          ) : null}
        </div>
